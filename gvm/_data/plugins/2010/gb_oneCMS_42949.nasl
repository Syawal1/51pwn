...
foreach dir( make_list_unique( "/cms", http_cgi_dirs( port:port ) ) ) {
  ...
  if( dir == "/" ) dir = "";
  url = string(dir, "/index.php?load=elite&view=1%3C/title%3E%3Cscript%3Ealert(%27", vt_strings["lowercase"], "%27)%3C/script%3E");

  if( http_vuln_check( port:port, url:url, pattern:"<script>alert\('" + vt_strings["lowercase"] + "'\)</script>", extra_check:"OneCMS", bodyonly:TRUE, check_header:TRUE ) ) {
