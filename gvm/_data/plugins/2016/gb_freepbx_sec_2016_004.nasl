...
function _run( file, port, host, vtstrings )
{
  local_var file, port;
...
buf = _run( file:file, port:port, host:host, vtstrings:vtstrings );
if( ! buf )
  exit( 0 );
...
if( buf =~ "uid=[0-9]+.*gid=[0-9]+" ) {
  ...
  if( d && strlen( d ) > 1 && d[0] == "/" ) {
    ...
    t = _run( file:file, port:port, host:host, vtstrings:vtstrings );
  }
