...
if (description)
{
  ...
  script_tag(name:"summary", value:"Apache Tomcat JK Connector (mod_jk) is prone to an authentication bypass vulnerability.");

  script_tag(name:"insight", value:"The Apache Web Server (httpd) specific code that normalised the requested path
...
foreach url (urls) {
  ...
  if (res =~ "^HTTP/1\.[01] 403") {
    ...
    if (res =~ "^HTTP/1\.[01] 200") {
      report = http_report_vuln_url(port: port, url: bp_check);
