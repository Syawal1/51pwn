...
if(description)
{
  ...
  script_version("2020-11-16T10:21:37+0000");
  script_tag(name:"last_modification", value:"2020-11-16 10:21:37 +0000 (Mon, 16 Nov 2020)");
...
function set_mandatory_key( key, regex, banner, extra_key ) {
  ...
  if( ! key ) {
    set_kb_item( name:"nvt_debug_empty/" + get_script_oid(), value:get_script_oid() + "#-#key#-#set_mandatory_key" );
    return;
  }

  if( ! regex ) {
    set_kb_item( name:"nvt_debug_empty/" + get_script_oid(), value:get_script_oid() + "#-#regex#-#set_mandatory_key" );
    return;
  }

  if( ! banner ) {
    set_kb_item( name:"nvt_debug_empty/" + get_script_oid(), value:get_script_oid() + "#-#banner#-#set_mandatory_key" );
    return;
  }
...
set_mandatory_key( key:"apache_or_server-info", regex:"^Server\s*:\s*Apache", banner:banner );
...
set_mandatory_key( key:"Basic_realm", regex:"WWW-Authenticate: Basic realm=", banner:banner );
...
set_mandatory_key( key:"DGN2200", regex:'WWW-Authenticate: Basic realm="NETGEAR DGN2200', banner:banner );
...
set_mandatory_key( key:"D-LinkDIR", regex:"^Server\s*:\s*(Linux, ((HTTP/1\.1)|(WEBACCESS/1\.0)), DIR|Mathopd|WebServer|lighttpd)", banner:banner );
set_mandatory_key( key:"D-LinkDNS", regex:"^Server\s*:\s*(lighttpd/|GoAhead-Webs)", banner:banner );
set_mandatory_key( key:"D-LinkDSL", regex:"^Server\s*:\s*(Boa|micro_httpd|Linux|RomPager)", banner:banner ); # For gb_dlink_dsl_detect.nasl
...
set_mandatory_key( key:"DSL_Router", regex:'WWW-Authenticate: Basic realm="DSL Router"', banner:banner );
...
set_mandatory_key( key:"HTTPserv", regex:"^Server\s*:\s*.*HTTPserv:", banner:banner );
...
set_mandatory_key( key:"JDownloader", regex:'WWW-Authenticate: Basic realm="JDownloader', banner:banner );
set_mandatory_key( key:"Jetadmin", regex:"HP Web Jetadmin", banner:banner );
set_mandatory_key( key:"JetBrainsIDEs", regex:"server: (PyCharm|WebStorm|CLion|DataGrip|IntelliJ IDEA|JetBrains MPS|jetBrains Rider|RubyMine)", banner:banner );
...
set_mandatory_key( key:"lighttpd", regex:"^Server\s*:\s*lighttpd", banner:banner );
...
set_mandatory_key( key:"mod_jk", regex:"Server\s*:\s*.+mod_jk", banner:banner );
set_mandatory_key( key:"mod_perl", regex:"Server\s*:\s*.+mod_perl", banner:banner );
set_mandatory_key( key:"mod_python", regex:"Server\s*:\s*.+mod_python", banner:banner );
...
set_mandatory_key( key:"Weborf", regex:"^Server\s*:\s*Weborf", banner:banner );
...
set_mandatory_key( key:"X-Mag", regex:"^X-Mag: .+", banner:banner );
