...
if(description)
{
  ...
  script_tag(name:"summary", value:"This script finds the running geccBBlite version.");
...
foreach dir( make_list_unique( "/", "/geccBBlite", "/geccBB", http_cgi_dirs( port:port ) ) ) {
  ...
  if( "geccBB - il bordo dei bollettini" >< res ) {

    version = "unknown";

    ver = eregmatch( pattern:"geccBBlite ([0-9.]+)", string:res );
    if( isnull( ver ) ) {
      ver = eregmatch( pattern:"geccBB - il bordo dei bollettini - v. ([0-9.]+)", string:res );
    }
