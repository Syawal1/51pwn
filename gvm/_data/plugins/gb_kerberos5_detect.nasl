...
if(description)
{
  ...
  script_tag(name:"summary", value:"This script detects the installed version of Kerberos5.");
  exit(0);
}
...
krbPath = ssh_find_bin(prog_name:"krb5-config", sock:sock);
foreach krbFile (krbPath)
{

  krbFile = chomp(krbFile);
  if(!krbFile)
    continue;

  krbVer = ssh_get_bin_version(full_prog_name:krbFile, version_argv:"--version", ver_pattern:"[Rr]elease ([0-9.]+)", sock:sock);
  if(krbVer[1] != NULL)
  {
    set_kb_item(name:"Kerberos5/Ver", value:krbVer[1]);
    log_message(data:"Kerberos5 version " + krbVer[1] + " running at location " + krbFile + " was detected on the host");
