...
foreach dir( make_list_unique( "/", http_cgi_dirs( port:port ) ) ) {
  ...
  if( dir == "/" ) dir = "";
  buf = http_get_cache(item:dir + "/search.php", port:port);
  if(!buf || buf !~ "^HTTP/1\.[01] 200" || "Comparison Shopping Script" >!<buf)
    continue;

  url = dir + "/search.php?s=%3Cscript%3Ealert(%27" + vt_strings["lowercase"] + "%27)%3C/script%3E";

  if( http_vuln_check( port:port, url:url, pattern:"<script>alert\('" + vt_strings["lowercase"] + "'\)</script>", check_header:TRUE, extra_check:"Comparison Shopping Script" ) ) {
