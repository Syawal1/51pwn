...
foreach dir (make_list_unique("/eventum", "/Eventum", "/", http_cgi_dirs(port:port)))
{

  if(dir == "/") dir = "";

  res = http_get_cache(item:string(dir,"/htdocs/index.php"), port:port);

  if(">Login - Eventum<" >< res)
  {

    url = string(dir,'/htdocs/forgot_password.php/"><script>alert("XSS-ATTACK_TEST")</script>');
    req = http_get(item:url, port:port);
    res = http_keepalive_send_recv(port:port, data:req);

    if(res =~ "^HTTP/1\.[01] 200" && '<script>alert("XSS-ATTACK_TEST")</script>' >< res)
    {
