...
seaName = ssh_find_file(file_name:"/seamonkey", useregex:TRUE, regexpar:"$", sock:sock);
if(!seaName)
  seaName = ssh_find_file(file_name:"/iceape", useregex:TRUE, regexpar:"$", sock:sock);
...
foreach binary_seaName(seaName) {
  ...
  seaVer = ssh_get_bin_version(full_prog_name:binary_name, version_argv:"-v", ver_pattern:"^Mozilla\sSeaMonkey\s([0-9]+\.[0-9.]+(\s(RC\s[0-9]+|Alpha|Beta))?)$");
  if(seaVer[1]) {
