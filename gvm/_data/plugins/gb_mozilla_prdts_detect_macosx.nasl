...
if(strlen(ffVerCmd) > 0 && "does not exist" >!< ffVerCmd){
  ...
  if(ffVer){
    ...
    foreach dir (key_list) {
      ...
      esrFile = ssh_find_file(file_name: dir + "/update-settings.ini", useregex:TRUE, regexpar:"$", sock:sock);
      if(esrFile) {
...
if(strlen(tbVerCmd) > 0 && "does not exist" >!< tbVerCmd){
  ...
  if(tbVer){
    thuFile = ssh_find_file(file_name:"/Applications/Thunderbird.app/Contents/MacOS/update-settings.ini", useregex:TRUE, regexpar:"$", sock:sock);
    if(thuFile) {
