...
foreach dir( make_list_unique( "/", "/admin-ng", http_cgi_dirs( port: port ) ) ) {
  ...
  foreach file( make_list( "/", "/login.html" ) ) {
    ...
    if( resp =~ "<title>Opencast[^<]{0,}" && ( 'version.version"> Opencast' >< resp || 'href="http://www.opencastproject.org"' >< resp  ||
                                               "<span>Welcome to Opencast</span>" >< resp ) ) {
      ...
      register_and_report_cpe( app: "Opencast", ver: version, concluded: version_match[0], base: "cpe:/a:opencast:opencast:", expr: '([0-9].[0-9].[0-9])', insloc: install, regService: "www", regPort: port, conclUrl: concluded_url );
