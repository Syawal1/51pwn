...
if(description)
{
  ...
  script_tag(name:"vuldetect", value:"Checks if a vulnerable version is present on the target host. If the IDE is Pycharm,
  send a crafted request via HTTP GET and POST and check the response.");

  script_tag(name:"insight", value:"Multiple flaws are due to Over-permissive CORS settings that allows attackers to use a
  malicious website in order to access various internal API endpoints, gain access to data saved by the IDE, and gather
  various meta-information like IDE version or open a project.");
...
if(ide == "^PyCharm") {
  ...
  if(!isnull(homePath))
    Pycharm_report = leakWithPyCharmHelpers(homePath:homePath, port:port);
  else {
    ProjectName = guessProjectName(port:port);
    if(!isnull(ProjectName))
      Pycharm_report = leakWithProject(name:ProjectName, port:port);
  }
}
