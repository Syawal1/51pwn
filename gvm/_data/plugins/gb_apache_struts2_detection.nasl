...
if(description)
{
  ...
  script_dependencies("find_service.nasl", "no404.nasl", "webmirror.nasl", "DDI_Directory_Scanner.nasl", "apache_server_info.nasl", "gb_apache_struts_detect.nasl", "global_settings.nasl");
  ...
  foreach url( make_list( dir, dir + "/struts2-blank", dir + "/struts2-basic",
    ...
    if( rcvRes =~ "^HTTP/1\.[01] 200" ) {
      ...
      if( "Struts2" >< rcvRes || "Apache Struts" >< rcvRes ) {
        ...
        app_report += '\n' + http_report_vuln_url( url:foundapp, port:port, url_only:TRUE );
      }
    }
  }
}
