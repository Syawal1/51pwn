...
if(description)
{
  ...
  script_version("2020-11-24T14:02:20+0000");
  script_tag(name:"last_modification", value:"2020-11-24 14:02:20 +0000 (Tue, 24 Nov 2020)");
...
function register_packages( buf ) {
  ...
  if( isnull( buf ) ) {
    set_kb_item( name:"nvt_debug_empty/" + get_script_oid(), value:get_script_oid() + "#-#register_packages#-#buf" );
    return NULL;
  }
...
function register_rpms( buf, custom_key_name ) {
  ...
  if( isnull( buf ) ) {
    set_kb_item( name:"nvt_debug_empty/" + get_script_oid(), value:get_script_oid() + "#-#register_rpms#-#buf" );
    return NULL;
  }
...
function create_lsc_os_detection_report( detect_text, no_lsc_support, rpm_access_error ) {
  ...
  if( isnull( detect_text ) ) {
    set_kb_item( name:"nvt_debug_empty/" + get_script_oid(), value:get_script_oid() + "#-#create_lsc_os_detection_report#-#detect_text" );
...
if( ": No such command" >< uname ) {
  ...
  if( system =~ "Version:\s*FAC" && "Architecture" >< system && "Branch point" >< system ) {
    ...
    register_and_report_os( os:"Fortinet FortiOS", cpe:"cpe:/o:fortinet:fortios", banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
...
if( "Amazon Linux AMI release" >< rls ) {
  ...
  register_and_report_os( os:"Amazon Linux", cpe:"cpe:/o:amazon:linux", banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
  exit( 0 );
}
...
if( rls =~ "oraclelinux-release-" ) {
  ...
  if( vers[1] ) {
    ...
    cpe += ":" + version;
    oskey += vers[1]; # nb: Special handling as the Oracle / ELSA LSCs are using just the major release

    log_message( port:port, data:create_lsc_os_detection_report( detect_text:os + version, rpm_access_error:error ) );
    register_and_report_os( os:os, version:version, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
  } else {
    log_message( port:port, data:create_lsc_os_detection_report( detect_text:os, rpm_access_error:error ) );
    register_and_report_os( os:os, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
  }
  set_kb_item( name:"ssh/login/release", value:oskey );
...
if( rls =~ "red hat linux release" ) {
  ...
  if( vers[1] ) {
    cpe += ":" + vers[1];
    oskey += vers[1];
    register_and_report_os( os:rls, version:vers[1], cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
  } else {
...
if( rls =~ "fedora" && rls =~ "release" ) {
  ...
  if( vers[2] ) {
    ...
    register_and_report_os( os:os, version:vers[2], cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
  } else {
    log_message( port:port, data:create_lsc_os_detection_report( detect_text:os, rpm_access_error:error ) );
    register_and_report_os( os:os, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
  }
...
if( rls =~ "mandriva" || rls =~ "mandrake" ) {
  ...
  if( vers[3] ) {
    ...
    register_and_report_os( os:os, version:vers[3], cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
    # nb: Additionally need to register Mandriva Enterprise Server 5 if a version of 5.0 has been detected
    if( vers[2] && vers[3] == "5.0" ) {
      register_and_report_os( os:os, version:"5", cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
    }
  } else {
    log_message( port:port, data:create_lsc_os_detection_report( detect_text:os, rpm_access_error:error ) );
    register_and_report_os( os:os, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
...
if( rls =~ "mageia release" ) {
  ...
  if( vers[1] ) {
    ...
    register_and_report_os( os:os, version:vers[1], cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
  } else {
    log_message( port:port, data:create_lsc_os_detection_report( detect_text:os, rpm_access_error:error ) );
    register_and_report_os( os:os, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
  }
...
if( ! isnull( match ) ) {
  ...
  if( ! isnull( buf ) ) {
    ...
    register_and_report_os( os:"Univention Corporate Server", version:match[2], cpe:"cpe:/o:univention:univention_corporate_server", banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
    exit( 0 );
  }
}
...
if( rls =~ "distrib_id=ubuntu" && rls =~ "distrib_release=" ) {
  ...
  if( vers[1] && vers[2] ) {
    ...
    register_and_report_os( os:os, version:version + lts, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
  } else {
...
if( rls =~ 'DISTRIB_ID=("|\')?Univention("|\')?' ) {
  ...
  if( ! isnull( ucs_release ) && ! isnull( ucs_description ) ) {
    ...
    log_message( port:port, data:create_lsc_os_detection_report( detect_text:ucs_desrciption[1] ) );

    set_kb_item( name:"ssh/login/release", value:"UCS" + ucs_release[1] );

    register_and_report_os( os:ucs_description[1], version:ucs_release[1], cpe:"cpe:/o:univention:univention_corporate_server", banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
  } else {
    register_unknown_os_banner( banner:'Unknown Univention release.\n\ncat /etc/lsb-release:\n\n' + rls, banner_type_name:SCRIPT_DESC, banner_type_short:"gather_package_list", port:port );
  }
  exit( 0 );
}
...
if( rls =~ "^[0-9]+[0-9.]+" || rls =~ "buster/sid" || rls =~ "bullseye/sid" ) {
  ...
  set_kb_item( name:"ssh/login/release", value:oskey );
  register_and_report_os( os:"Debian GNU/Linux", version:rls, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
...
if( rls =~ "slackware" ) {
  ...
  if( vers[1] ) {
    ...
    register_and_report_os( os:"Slackware", version:vers[1], cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
  } else {
    log_message( port:port, data:create_lsc_os_detection_report( detect_text:"Slackware" ) );
    register_and_report_os( os:"Slackware", cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
  }
  set_kb_item( name:"ssh/login/release", value:oskey );
...
if( 'NAME="Arch Linux"' >< rls ) {
  ...
  register_and_report_os( os:"Arch Linux", cpe:"cpe:/o:archlinux:arch_linux", banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
  exit( 0 );
}
...
if( rls =~ "suse linux enterprise desktop " ) {
  ...
  if( vers[1] ) {
    ...
    register_and_report_os( os:os, version:vers[1], patch:"SP" + patchlevel, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
  } else {
    log_message( port:port, data:create_lsc_os_detection_report( detect_text:os, rpm_access_error:error ) );
    register_and_report_os( os:os, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
  }
...
if( rls =~ "suse linux enterprise server " ) {
  ...
  if( vers[1] ) {
    ...
    log_message( port:port, data:create_lsc_os_detection_report( detect_text:os, rpm_access_error:error ) );
    register_and_report_os( os:os, version:version, patch:"SP" + patchlevel, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide", full_cpe:TRUE );
  } else {
    log_message( port:port, data:create_lsc_os_detection_report( detect_text:os, rpm_access_error:error ) );
    register_and_report_os( os:os, cpe:cpe, banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
  }
...
if( "Gentoo" >< rls ) {
  ...
  register_and_report_os( os:"Gentoo", cpe:"cpe:/o:gentoo:linux", banner_type:"SSH login", desc:SCRIPT_DESC, runs_key:"unixoide" );
