...
if(description)
{
  ...
  script_dependencies("gb_get_http_banner.nasl", "no404.nasl", "DDI_Directory_Scanner.nasl", "webmirror.nasl");
...
function initialize_dirs () {
  ...
  foreach d( make_list_unique( "/scripts", "/msadc", http_cgi_dirs( port:port ) ) ) {
    add_dir_list(dir:d);
    _dir_idx = _dir_idx + 1;
  }
  return(_dir_idx);
