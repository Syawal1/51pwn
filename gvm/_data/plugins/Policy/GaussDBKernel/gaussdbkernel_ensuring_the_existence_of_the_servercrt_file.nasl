...
if(description)
{
  script_oid("1.3.6.1.4.1.25623.1.0.150407");
  ...
  script_name("GaussDB Kernel: Ensuring the Existence of the server.crt File");
...
ensure that the server certificate is verified on the client. Therefore, the server must be configured to use only the hostssl connection, and the server.key (key)
...
if( ! get_kb_item( "login/SSH/success" ) || ! sock = ssh_login_or_reuse_connection() ) {
  ...
  compliant = "no";
  comment = "Please check the value.";
}
