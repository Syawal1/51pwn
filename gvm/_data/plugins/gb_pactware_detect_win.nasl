...
if(description)
{
  ...
  script_version("2020-09-24T08:52:46+0000");
  script_tag(name:"last_modification", value:"2020-09-24 08:52:46 +0000 (Thu, 24 Sep 2020)");
...
foreach key( key_list ) {
  foreach item( registry_enum_keys( key:key ) ) {
    ...
    if( ! appName || appName !~ "PACTware [0-9.]+" )
      continue;
    ...
    if( dispvers = registry_get_sz( key:key + item, item:"DisplayVersion" ) ) {
      if( appName =~ "SP" ) {
        strip_vers = eregmatch( pattern:"(([0-9.]+) (SP[0-9])?)", string:appName );
